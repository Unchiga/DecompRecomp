...
static inline __attribute__((always_inline)) void plot(int x, int y, int r, int g, int b, int u, int v, int flags)
{
    ...
    if (TextureDump_Tags) { /* drawn, not from the disc */
        TextureDump_Tags[target - vram] = 0;
        if (TextureDump_Shadow) memset(TextureDump_Shadow + (target - vram) * 4, 0, 4 * sizeof(uint16_t));
    }
    if (flags & 4) {
        source = texel(u, v);
        if (!source) {
