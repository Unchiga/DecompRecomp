...
rem  - tmp\pc\hang-*.txt (and .dmp) when the game stops calling VSync for 5 s (window frozen),
rem  - tmp\pc\crash-*.txt (and .dmp) when it crashes, however it ends,
rem  - tmp\pc\last-session.log, everything the game printed.
...
move /y tmp\pc\hang-*.* tmp\pc\debug\previous\ >nul 2>&1
move /y tmp\pc\crash-*.* tmp\pc\debug\previous\ >nul 2>&1
...
rem Windows' own dump as well, when local dumps are on (crash_report.py).
